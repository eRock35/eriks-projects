// Spar's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, database `spar` (Native mode). What Cloud Run uses.
//   - Memory, when SPAR_MEMORY=1. What tests and a laptop use. It exists so the
...
//     that restarts whenever it scales to zero would lose everyone's history
//     and look fine doing it.
//
// Collections are paths: `players/<uid>/sessions` is a real Firestore
// subcollection, and the memory backend just keys on the same string. Keeping
// history under the player means "my sessions, newest first" is a single-field
// order and needs no composite index.

const MEMORY = process.env.SPAR_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('SPAR_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
function firestoreBackend() {
  ...
  const db = new Firestore({
    ...
    databaseId: process.env.FIRESTORE_DATABASE_ID || 'spar',
