...
async function design(table, hint = '', model = MODEL, useClient = null) {
  const res = await (useClient || anthropic()).messages.create({
    model,
    max_tokens: 2048,
    system: SYSTEM,
    tools: [TOOL],
...
async function tableFromProse(prose, hint = '', model = MODEL, useClient = null) {
  const res = await (useClient || anthropic()).messages.create({
    model,
    max_tokens: 4096,
