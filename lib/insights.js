...
const APPS = [
  ...
  { key: 'friction', label: 'Friction', db: 'friction', doc: 'control/last-run', field: 'finishedAt', staleHours: 26, note: 'daily scan' },
...
async function users() {
  ...
  // Anyone who asked for access and has not been answered. These are the rows
  // a person is actually waiting on, so they get their own place rather than
  // being buried in a table.
  const pending = [];
  rows.forEach((u) => {
    const reqs = u.requests && typeof u.requests === 'object' ? u.requests : {};
    Object.entries(reqs).forEach(([app, r]) => {
      if (!r || r.state === 'denied' || !r.at) return;
      if (u.access && u.access[app]) return;
      pending.push({ uid: u.id, email: u.email || null, app, at: r.at, note: r.note || '' });
    });
  });
  ...
  return {
    ...
    list: rows
      .map((u) => ({
        ...
        // Which apps this account may actually use. Identity says who they
        // are; this says what that buys them, and it is the only thing on
        // this row the admin can change.
        access: u.access && typeof u.access === 'object' ? u.access : {},
        requests: u.requests && typeof u.requests === 'object' ? u.requests : {},
        budget: identityLib.budgetFor(u),
      }))
