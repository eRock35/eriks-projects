...
//
// Its hostname is NOT written here. This repo is public, and that app's rule
// is "no literal URL in a public file" - scrapers index strings, they don't
// run inferences. It comes from SANTAROSA_HEALTH_URL on the deployed service.
// Unset, the target probes as a failure rather than quietly vanishing from
// the board: an app nobody is watching should look wrong, not look fine.
//
...
const TARGETS = [
  ...
  { key: 'santarosa',  label: 'Santa Rosa',       url: process.env.SANTAROSA_HEALTH_URL || '' },
...
async function probe(t) {
  ...
  if (!t.url) return { ...t, ok: false, status: 0, ms: 0, error: 'no health URL configured' };
...
async function check() {
  ...
  for (const r of results) {
    ...
    // lastError and lastBody are always written, empty when there is nothing
    // to say. The write below is a merge, and a merge does not delete keys the
    // new map leaves out - so omitting them on a healthy probe left the last
    // failure's body sitting on a record now reporting HTTP 200. That reads as
    // a broken app to anyone looking at the document.
    next[r.key] = {
      fails,
      alerted,
      lastStatus: r.status,
...
module.exports = { check, compose, probeOne: probe, TARGETS, FAILURES_BEFORE_ALERT };
