...
const TARGETS = [
  { key: 'football',   label: 'College Football', url: 'https://college-football-app-u4h4ftn3fa-uc.a.run.app/healthz' },
  { key: 'trip',       label: 'Trip Planner',     url: 'https://trip-planner-u4h4ftn3fa-uc.a.run.app/healthz' },
  { key: 'dataviz',    label: 'DataViz',          url: 'https://dataviz-u4h4ftn3fa-uc.a.run.app/healthz' },
  { key: 'friction',   label: 'Friction',         url: 'https://friction-u4h4ftn3fa-uc.a.run.app/healthz' },
  { key: 'hopscotch',  label: 'Hopscotch',        url: 'https://hopscotch-u4h4ftn3fa-uc.a.run.app/api/health' },
  { key: 'santarosa',  label: 'Santa Rosa',       url: 'https://santa-rosa-host.invalid/healthz' },
  { key: 'landing',    label: 'Landing & blog',   url: 'https://landing-page-u4h4ftn3fa-uc.a.run.app/healthz' },
];
...
async function probe(t) {
  ...
  try {
    ...
    return { ...t, ok, status: res.status, ms: Date.now() - started };
  } catch (err) {
...
async function check() {
  ...
  for (const r of results) {
    ...
    next[r.key] = { fails, alerted, lastStatus: r.status, lastMs: r.ms, lastCheckedAt: new Date().toISOString(), ...(r.error ? { lastError: r.error } : {}) };
  }
...
function compose({ nowDown, recovered }) {
  ...
  const html = [];
  if (nowDown.length) {
    lines.push('NOT ANSWERING', ...nowDown.map((r) => `  ${r.label} - ${r.error || 'HTTP ' + r.status}`));
