...
function compose(found, origin) {
  const dash = `${origin}/admin/insights`;
  const lines = [];
  const html = [];
  ...
  section('Waiting on you', found.requests, (r) => ({
    text: `${r.email} asked for AI access (${String(r.at).slice(0, 16).replace('T', ' ')})${r.note ? ` — "${r.note}"` : ''}`,
    html: `<b>${esc(r.email)}</b> asked for AI access on Trip Planner${r.note ? ` — <i>${esc(r.note)}</i>` : ''}`,
  }));
