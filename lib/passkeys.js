...
function create({ store, tokens, rpName, userName, issueSession, requireAdmin, adminPasswordOk }) {
  ...
  function mount(app) {
    ...
    app.post('/api/admin/passkey/register/options', requireAdmin, async (req, res) => {
      try {
        // MUST be awaited. adminPasswordOk is async now (the current password
        // may be the stored one rather than the env var), and `!promise` is
        // always false - leaving this synchronous would have let any signed-in
        // session enrol a passkey with no password at all.
        if (!(await adminPasswordOk(String((req.body && req.body.password) || '')))) {
