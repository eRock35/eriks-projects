// Booth's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, the lab database `challenge` (Native mode). What Cloud Run uses.
//   - Memory, when BOOTH_MEMORY=1. What tests and a laptop use. It exists so the
...
//     that restarts whenever it scales to zero would lose every show's leads
//     and look fine doing it.
//
// Collections are paths: `events/<id>/leads` is a real Firestore
// subcollection, and the memory backend just keys on the same string. Each
// lead is its own document under the event, so two people capturing at the
// same moment write two different documents and can never overwrite each
// other. "My events" is one array-contains query on `memberIds`, which
// Firestore indexes by itself - no composite index.

const MEMORY = process.env.BOOTH_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('BOOTH_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
function memoryBackend() {
  ...
  return {
     ...
     *  Firestore's arrayUnion / arrayRemove, so two people joining an event at
...
// Booth shares the lab's database with its sibling apps; a prefix keeps its
// collections apart there (`booth_events`, `booth_codes`, ...). Letters, digits
// and underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.BOOTH_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');
