// An invoice photo: checked on the way in, read once by the model, dropped.
//
...
// survives is the proposal the person edits and chooses to save. Same
// read-once rule as Snapquote's job photos and trip-planner's receipts.
...
function validate(raw) {
  if (!raw || typeof raw !== 'object') throw httpError(400, 'Add a photo of the invoice.');
  ...
  if (!data || !/^[A-Za-z0-9+/=\s]+$/.test(data)) throw httpError(400, 'That photo could not be read. Try a JPEG or PNG.');
  ...
  if (Math.floor(data.replace(/\s/g, '').length * 3 / 4) > MAX_BYTES + 3) {
    throw httpError(400, 'That photo is too large (4 MB max). The app shrinks photos before sending - try again from the page.');
  }
  const buf = Buffer.from(data, 'base64');
  if (!buf.length) throw httpError(400, 'That photo is empty.');
  if (buf.length > MAX_BYTES) throw httpError(400, 'That photo is too large (4 MB max).');
