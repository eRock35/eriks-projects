// Tally's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, the lab database `challenge` (Native mode). What Cloud Run uses.
//   - Memory, when TALLY_MEMORY=1. What tests and a laptop use. It exists so
...
//     closed day and every deposit and look fine doing it.
//
// Collections are paths: `days/<uid>/items/<date>` is a real Firestore
// subcollection, and the memory backend just keys on the same string. A
// person's closed days, deposits and settings live under their own uid, so
// nobody else's id can even name them. Each day and each deposit is its own
// document, so importing a statement while closing today writes different
// documents and can never overwrite each other.

const MEMORY = process.env.TALLY_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('TALLY_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
// Tally shares the lab's database with its sibling apps; a prefix keeps its
// collections apart there (`tally_days`, `tally_deposits`, ...). Letters,
// digits and underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.TALLY_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');
