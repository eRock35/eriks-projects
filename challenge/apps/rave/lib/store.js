// Rave's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, the lab database `challenge` (Native mode). What Cloud Run uses.
//   - Memory, when RAVE_MEMORY=1. What tests and a laptop use. It exists so the
...
//     that restarts whenever it scales to zero would lose everyone's reviews
//     and look fine doing it.
//
// Collections are paths: `reviews/<uid>/items` is a real Firestore
// subcollection, and the memory backend just keys on the same string. Keeping
// each person's reviews under their uid means "mine" is a path,
// not a query, and needs no composite index.

const MEMORY = process.env.RAVE_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('RAVE_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
// Rave shares the lab's database with its sibling apps; a prefix keeps its
// collections apart there (`rave_reviews`, `rave_shares`, ...). Letters, digits
// and underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.RAVE_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');
