// Pop Quiz's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, the lab database `challenge` (Native mode). What Cloud Run uses.
//   - Memory, when POPQUIZ_MEMORY=1. What tests and a laptop use. It exists so the
//     whole app - sign-in included - can be driven end to end with no project,
//     no key and no network. It is refused on Cloud Run (K_SERVICE is set
//     there), because a deployment that silently kept its data in a process
//     that restarts whenever it scales to zero would lose every team's streaks
//     and look fine doing it.
//
// Collections are paths: `teams/<id>/progress` is a real Firestore
// subcollection, and the memory backend just keys on the same string. Each
// member's progress is its own document under the team, so two people
// answering at the same moment write two different documents and can never
// overwrite each other. "My teams" is one array-contains query on
// `memberIds`, which Firestore indexes by itself - no composite index.

const MEMORY = process.env.POPQUIZ_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('POPQUIZ_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
function memoryBackend() {
  ...
  return {
     ...
     *  Firestore's arrayUnion / arrayRemove, so two people joining a team at
     *  once both end up in memberIds. */
...
// Pop Quiz shares the lab's database with its sibling apps; a prefix keeps its
// collections apart there (`popquiz_teams`, `popquiz_codes`, ...). Letters, digits
// and underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.POPQUIZ_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');
