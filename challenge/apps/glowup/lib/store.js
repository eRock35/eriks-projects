// Glowup's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, the lab database `challenge` (Native mode). What Cloud Run uses.
//   - Memory, when GLOWUP_MEMORY=1. What tests and a laptop use. It exists so the
...
//     that restarts whenever it scales to zero would lose every seller's
//     versions and look fine doing it.
//
// Collections are paths: `listings/<uid>/items` is a real Firestore
// subcollection, and the memory backend just keys on the same string. A
// person's listings live under their uid, so "mine" is a path, not a query -
// no composite index, and nobody else's id can even be named in one. Each
// version is its own document under its listing, so saving never rewrites
// the history.

const MEMORY = process.env.GLOWUP_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('GLOWUP_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
function memoryBackend() {
  ...
  return {
     ...
     *  Firestore's arrayUnion / arrayRemove, so two saves on one day from two
     *  tabs both land in the improvement days. */
...
// Glowup shares the lab's database with its sibling apps; a prefix keeps its
// collections apart there (`glowup_listings`, `glowup_shares`, ...). Letters, digits
// and underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.GLOWUP_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');
