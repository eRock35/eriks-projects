// Snapquote's own data, and a memory stand-in for running it without Google.
//
// Two backends behind one small interface:
//
//   - Firestore, database `snapquote` (Native mode). What Cloud Run uses.
//   - Memory, when SNAPQUOTE_MEMORY=1. What tests and a laptop use. It exists so the
...
//     that restarts whenever it scales to zero would lose everyone's quotes
//     and look fine doing it.
//
// Collections are paths: `pros/<uid>/quotes` is a real Firestore
// subcollection, and the memory backend just keys on the same string. Keeping
// quotes under the pro means "my quotes, newest first" is a single-field
// order and needs no composite index.

const MEMORY = process.env.SNAPQUOTE_MEMORY === '1';
if (MEMORY && process.env.K_SERVICE) {
  throw new Error('SNAPQUOTE_MEMORY=1 is for local use only and is refused on Cloud Run.');
}
...
// Snapquote may share a database with sibling apps in a combined host
// service; a prefix keeps its collections apart there. Letters, digits and
// underscores only, so it can never introduce a path separator.
const PREFIX = String(process.env.SNAPQUOTE_COLLECTION_PREFIX || '').replace(/[^a-z0-9_]/gi, '');

function firestoreBackend() {
  const { Firestore, FieldValue } = require('@google-cloud/firestore');
  const db = new Firestore({
    projectId: process.env.GOOGLE_CLOUD_PROJECT,
    databaseId: process.env.FIRESTORE_DATABASE_ID || 'snapquote',
