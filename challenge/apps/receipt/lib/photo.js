// One whiteboard photo for the recap writer: checked on the way in, read
// once by the model, dropped.
//
...
// survives is the proposed recap the facilitator reads, edits and copies -
// and even that is not stored. A whiteboard after a meeting often holds
// names, numbers and plans that were never meant to leave the room; the photo
// of it is not ours to keep. Same read-once rule as Booth's card photos and
// Glowup's screenshots.
...
function validate(raw) {
  if (!raw || typeof raw !== 'object') throw httpError(400, 'Add a photo of the whiteboard.');
  ...
  if (Math.floor(data.replace(/\s/g, '').length * 3 / 4) > MAX_BYTES + 3) {
    throw httpError(400, 'That image is too large (4 MB max). The app shrinks photos before sending - try again from the page.');
  }
